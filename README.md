...
* 그 문제는 일단 위에서도 기록했듯이 (password can be not null) 인데, 일단 구글 oauth의 반환값에는 비밀번호가 오지 않는다. 그리고 가장 궁극적인 원인은 회원 정보를 저장하는 Entity에서 password 필드를 nullable=false 라고 한것이다. 정리하자면 구글 oAuth는 비밀번호를 반환하지 않는데 난 DB에 그 값을 저장할려고 했던거였다. 그 사실을 알고 있었다 하더라고 password 필드를 not null 이라고 해놓아서 오류가 발생한것이다. 
