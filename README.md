...
* 구글 oAuth 기능 구현 중, 구글 로그인 버튼을 클릭하면 구글 로그인이 되도록 구현해놨는데 맨 처음에는 mismatch uri 라고 해서 GCP 쪽 설정 문제인것 같아 uri를 통일해 주었다 그래서 구글 로그인 화면이 한번 떠서 내 계정을 클릭하니깐 password can br not null이 뜨고 그 다음부터 구글 로그인 화면이 안나오고 Spring Security 쪽 로그인 화면이 계속 뜬다. -> 코드 문제는 아닌것 같아 GCP 사용법을 더 찾아봐야겠다  
